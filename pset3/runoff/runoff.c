...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count ; i++)
    {
        ...
        {
            top_preference = preferences[i][k];
            if (candidates[top_preference].eliminated != true)
            {
...
{
    // TODO
    int count = voter_count / 2;
    for (int i = 0 ; i < candidate_count ; i++)
    {
        string most_votes = candidates[i].name;
        if (candidates[i].eliminated != true && candidates[i].votes > count)
        {
            printf("%s\n", most_votes);
            return true;
        }
